    ...
    <div className="w-full space-y-4">
      ...
      <form onSubmit={handleSearch} className="relative flex items-center">
        <input
          ...
          className="w-full bg-[#0d1527] border border-slate-700/70 rounded-xl px-4 py-3 pr-24 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500 transition-colors"
        />
        <button
          type="submit"
      ...
      {hasSearched && (
        <div className="pt-2">
          ...
          {results.length === 0 ? (
            <p className="text-xs text-slate-500 italic">No matching chunks found.</p>
          ) : (
            <div className="space-y-3">
              {results.map((item, index) => (
                <div
                  key={index}
                  className="bg-[#0d1527] border border-slate-800 rounded-xl p-4 space-y-2 hover:border-slate-700 transition-colors"
                >
                  {/* Score & Page Badge */}
